const { HeroMetric, SupportingMetric, TrendDelta, Qualifier, TrajectoryChart, ChartLegend, RangeStrip,
        SegmentedControl, Switch, Prose, TextLink, Tooltip, Icon } = window.HealthTrendDesignSystem_5b333a;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-10)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 'var(--space-9)', flexWrap: 'wrap' }}>
        {/* Precision, units and dates come from HTFormat — never formatted inline.
            digits reserves 5 character slots so kg -> lb does not reflow the page. */}
        <HeroMetric label="Estimated trend weight"
          value={F.trendWeight(last.y)} unit={F.getLocale().unit}
          digits={F.digitSlots()}
          interval={F.plusMinus(last.hi68 - last.y, { level: 68 })}
          asOf="as of today, 06:40" />
        {/* Rate is hero-ADJACENT: same region as the trajectory, never inside the
            tier-2 statistics group below. */}
        <div style={{ display: 'flex', gap: 'var(--space-9)', alignItems: 'flex-end', paddingBottom: 6 }}>
          <TrendDelta value={F.signed(-0.27, 2)} unit={F.UNITS[F.getLocale().unit].rate} direction="down"
            interval={F.rateInterval(-0.34, -0.19, { level: 95 })} />
        </div>
      </div>
